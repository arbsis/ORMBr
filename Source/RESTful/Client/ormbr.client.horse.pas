...
uses
  ormbr.client.resthorse.factory;
...
begin
  inherited;
  FRESTFactory := TRESTFactoryHorse.Create(Self);
  FRESTClient := TRESTClient.Create(Self);
  FRESTRequest := TRESTRequest.Create(Self);
  FRESTResponse := TRESTResponse.Create(Self);
  FRESTRequest.Client := FRESTClient;
  FRESTRequest.Client.ContentType := cCONTENTTYPE;
//  'http://localhost:9000/api/ormbr/master?%24top=3&%24skip=9'
  FRESTRequest.Response := FRESTResponse;
  FRESTResponse.RootElement := '';
  FRESTResponse.ContentType := cCONTENTTYPE;
  FAPIContext := 'api';
  FRESTContext := '';
  // Monta a URL base
  SetBaseURL;
...
//  FRESTRequest.AddParameter('_charset', 'UTF-8');
  // Params
  for LFor := 0 to FParams.Count -1 do
  begin
