...
begin
  ...
  if ATypeInfo = TypeInfo(Nullable<SmallInt>) then
    if TVarData(AValue).VType <= varNull then
      Self.SetValue(AInstance, TValue.From(Nullable<SmallInt>.Create(AValue)))
    else
      Self.SetValue(AInstance, TValue.From(Nullable<SmallInt>.Create(SmallInt(AValue))))
  else
  if ATypeInfo = TypeInfo(Nullable<String>) then
